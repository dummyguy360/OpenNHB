function __input_finalize_default_profiles()
{
    ...
    if (_global.__touch_allowed)
        variable_struct_set(_global.__default_profile_dict, "touch", variable_struct_get(_global.__default_profile_dict, "touch") ?? {});
    ...
    repeat (array_length(_global.__profile_array))
    {
        ...
        repeat (array_length(_profile_verb_names))
        {
            ...
            if (array_length(_verb_data) > 3)
                __input_error("Verb \"", _verb_name, "\" for default profile \"", _profile_name, "\" has too many alternate bindings (", array_length(_verb_data), " versus max ", 3, ")\nPlease increase INPUT_MAX_ALTERNATE_BINDINGS if you'd like to use more alternate bindings");
